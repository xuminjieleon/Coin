...
> 会话问答归档（非开发内容：概念解释/实操建议/方法论等结论式存档）在 **`QA.md`**——按日期顶部追加，新问答只存结论。
...
**策略到底赚不赚钱**（回测口径、未计费）：8 个主流币 × 5 年 × 四个周期，每个币合计 +559~+918R，**每币每一年都赚**（含 2022 大熊市）。1R=每笔准备亏的钱，按单笔风险=本金 1% 换算，五年毛利润 ≈ 本金的 +559%~+918%（未复利）。方向猜对猜错接近抛硬币——钱靠"等回调才进、错了亏小钱、对了先保本、利润让它跑"的执行纪律赚，这个优势在几十个币、A 股、美股上都复现过，是结构性的、不挑市场。

**实盘要打几折**：费用双边 0.10% 时利润保留 1h 57% / 4h 88% / 1d 96%——1h 必须挂单（maker）入场，账面利润还要再按下界口径缩水（BACKTEST §4.3 附表）；复利表里"几千倍"是数学上限不可当真，最可实现的复利口径是 1d：f=1% 净费五年 8.1×、年化 +72%。
...
**微信推送**：企业微信通道，每小时把 8 币 × 1h/4h 的新计划/计划转向/计划消失/**改单（entry/stop 漂移）**推到手机，只认收盘数据，消息只含实际挂单价；【消失】/【转向】带显式撤单指令。已双机运行。

**已知短板（诚实）**：方向胜率天花板 ~60%（多轮验证）；1w 周期样本太小不可靠；清算/订单簿是快照非实时；宏观数据要开 VPN；参数已冻结不再调（防过拟合，无新数据不重开）。
...
5. **用户验收微信推送（2026-08-27 第十七轮）**：pushplus.plus 扫码登录复制 token → `POST /api/notify {"token":"...","enabled":true}` → `POST /api/notify/test` 收到测试消息 → 等下一个整点+5min 确认调度运行（events 模式首轮静默播种，事件从第二轮起推）。**当前状态（第二十二/二十七轮）**：部署机与本机均已配置企业微信通道且 enabled=true，intervals=["1h","4h"]——**双机冗余推送**（两机指纹独立、配置需分别改、偶发重复消息属预期）；**2026-08-29 第三十四/三十六轮起 symbols=BTC/ETH/SOL/BNB/XRP/ZEC/DOGE/SUI（4→8，本机已改，部署机待同步）**；待用户在手机端确认收到带周期标签的事件消息即可关闭此项
...
11. **已关闭（2026-08-31 第四十七轮裁决）**：Wyckoff spring/utad 死代码修复线——预登记验证修复被数据否决（K1 独挂），死代码按证据维持现状，不再开第三轮（除非新数据维度）。**遗留待办**：①正典 `_5y_cache_*` 记录（08-28 构建）与当前 K 线内容存在历史漂移（第四十七轮诊断，分币混合方向差、池化 +1.3~2.3%）——是否停机重建正典缓存待用户拍板（重建=全部下游报告数字按新窗口重呈现）；②回测评估窗口钉 end_time 已成标准（活进程实时追加 kline 缓存的坑）
12. 未排期（第四十七轮复核确认的剩余收益路径，**待用户拍板**）：标的池扩展——PYTH 第七轮纯样本外 +274.9R（EV+0.172R、PF 4.82）验证过却不在推送列表；§6.2 实测池化利润按标的数量近线性扩展（新增 4 币贡献 1h +48%）。扩列表=notify.json 配置改动+按第三十六轮 SUI 流程补回测档，零策略改动
